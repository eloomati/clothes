...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, images, labels, batch_size=64, shuffle=False, augment=False):
        self.labels = labels  if labels is not None else [] # array of labels
        self.images = images  if images is not None else []# array of images
    ...
    def on_epoch_end(self):
        # wywyływana po zakończeniu każdej epoki tren. lub test. i służy do aktualizacji indeksów danych
    ...
    def generate(self, data_path, subset, img_height, img_width):
        ds = keras.utils.image_dataset_from_directory(
            data_path,
            labels='inferred',
            validation_split=0.2 if subset == 'training' else 0.5,
            seed=110,
            subset=subset,
...
def real_split():
    ...
    train_generator = DataGenerator(
        ...
        shuffle=False,  # Opcjonalnie można ustawić na True, jeśli potrzebujesz losowego tasowania
        augment=True  # Opcjonalnie można ustawić na True, jeśli chcesz stosować augmentację
    )

    train_images, train_labels = train_generator.generate(data_path, 'training', img_height, img_width)
    ...
    val_generator = DataGenerator(
        ...
        batch_size=batch_size,
        shuffle=False,  # Opcjonalnie można ustawić na True, jeśli potrzebujesz losowego tasowania
        augment=False  # Opcjonalnie można ustawić na True, jeśli chcesz stosować augmentację
    )

    val_images, val_labels = val_generator.generate(data_path, 'validation', img_height, img_width)

    test_data_path = v.data_path + '/test'
    ...
    test_images, test_labels = test_generator.generate(test_data_path, 'both', img_height, img_width)
